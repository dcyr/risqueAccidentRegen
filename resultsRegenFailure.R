...
#                harvestTreatment = harvestRate) %>%
#         group_by(scenario, harvestTreatment, simID, timestep, fireZone, cover, burnedImmature) %>%
...
#     
#     dfSummary <- dfSummary %>%
#         select(scenario, harvestTreatment, simID, timestep, fireZone, cover, burnedImmature, area_ha) %>%
...
dfCover <- dfSummary %>%
    ...
    #filter(simID == "000") %>%
    group_by(scenario, harvestTreatment, simID, cover, timestep, burnedImmature) %>%
    summarize(area_ha = sum(area_ha)) %>%
    merge(dfAreaCover) %>%
    mutate(propBurned = area_ha/areaCover_ha) %>%
    ungroup() %>%
    arrange(timestep) %>%
    group_by(scenario, harvestTreatment, simID, cover, burnedImmature) %>%
...
dfCoverTotal <- dfCover %>%
    group_by(scenario, harvestTreatment, simID, timestep, burnedImmature) %>%
    summarize(area_ha = sum(area_ha),
              areaCover_ha = sum(dfAreaCover$areaCover_ha)) %>%
    mutate(propBurned = area_ha/areaCover_ha) %>%
    ungroup() %>%
    arrange(timestep) %>%
    group_by(scenario, harvestTreatment, simID, burnedImmature) %>%
    mutate(cumulpropBurned = cumsum(propBurned),
           cover = "Global") 
...
dfCover <- dfCover %>%
    group_by(scenario, harvestTreatment, cover, simID) %>%
...
dfCoverPad <- dfCover %>%
    group_by(cover, harvestTreatment, scenario, simID) %>%
...
dfCoverPercentiles <- dfCover %>%
    group_by(scenario, harvestTreatment, timestep, cover) %>%
...
for (ts in 1:50) {
    dfCoverPercentiles <- dfCoverPercentiles %>%
        filter(timestep <= ts) %>%
        group_by(scenario, harvestTreatment, cover) %>%
...
dfCoverPercentilesDecade <- as.data.frame(dfCoverPercentiles) %>%
    ...
    rbind(dfTmp) %>%
    arrange(scenario, harvestTreatment, cover, timestep) %>%
    group_by(scenario, harvestTreatment, cover) %>%
...
dfCoverPercentilesDecade <- select(dfCoverPercentilesDecade,
                                   scenario, harvestTreatment, timestep, cover,
                                   diff_mean,
                                   diff_p.050,
                                   diff_p.100,
                                   diff_p.250,
...
labelDF <- dfCoverPercentiles %>%
    filter(timestep == 50) %>%
    select(cover, harvestTreatment, timestep, medianRate)
...
for (s in c("baseline", "RCP85")) {
    ...
    for (i in c("all", "Global")) {
        ...
        #labels$harvestTreatment <- factor(labels$harvestTreatment, levels = c("sans récolte", "avec récolte"))
        #df$harvestTreatment <- factor(df$harvestTreatment, levels = c("sans récolte", "avec récolte"))
        ...
        m <- ggplot(df, aes(x=timestep + 2015, y = cumulpropBurned, group = ID)) +
            ...
            geom_line(aes_string(y = names(p)[4], group = 1),
                      colour = "yellow",
                      linetype = 4, size = 0.4, alpha = 1) +
            facet_grid(cover~harvestTreatment) +
            theme_dark() #+
...
dfZone <- dfSummary %>%
    filter(burnedImmature == T) %>%
    group_by(scenario, harvestTreatment, simID, Zone_LN, timestep) %>%
    summarize(area_ha = sum(area_ha)) %>%
    merge(dfAreaZone) %>%
    mutate(propBurned = area_ha/areaZone_ha) %>%
    ungroup() %>%
    arrange(timestep) %>%
    group_by(scenario, harvestTreatment, simID, Zone_LN) %>%
    mutate(cumulpropBurned = cumsum(propBurned)) 


dfZoneTotal <- dfZone %>%
    group_by(scenario, harvestTreatment, simID, timestep) %>%
    summarize(area_ha = sum(area_ha),
              areaZone_ha = sum(dfAreaZone$areaZone_ha)) %>%
    mutate(propBurned = area_ha/areaZone_ha) %>%
    ungroup() %>%
    arrange(timestep) %>%
    group_by(scenario, harvestTreatment, simID) %>%
    mutate(cumulpropBurned = cumsum(propBurned),
           Zone_LN = "Global") 
...
# computing mean regen failure rates per simulation
dfZone <- dfZone %>%
    group_by(scenario, harvestTreatment, Zone_LN, simID) %>%
    summarise(meanRate = sum(propBurned)/50) %>%
    merge(dfZone)

### pad dfCover at timestep == 50 for when there's been no fire
dfZonePad<- dfZone %>%
    group_by(Zone_LN, harvestTreatment, scenario, simID) %>%
    filter(timestep == timestep[which.max(timestep)]) %>%
    ungroup() %>%
    mutate(area_ha = ifelse(timestep<50, 0, area_ha),
           propBurned = ifelse(timestep<50, 0, propBurned)) %>%
...
dfZonePercentiles <- dfZone %>%
    group_by(scenario, harvestTreatment, timestep, Zone_LN) %>%
    summarise(p.050 = quantile(cumulpropBurned, 0.05, na.rm = T),
              p.100 = quantile(cumulpropBurned, 0.1, na.rm = T),
              p.250 = quantile(cumulpropBurned, 0.25, na.rm = T),
              p.500 = quantile(cumulpropBurned, 0.5, na.rm = T),
...
labelDF <- dfZonePercentiles %>%
    filter(timestep == 50) %>%
    select(Zone_LN, scenario, harvestTreatment, timestep, medianRate)
# 
# dfTmp <- list()
# i <- 1
# for (s in unique(dfZonePercentiles$scenario)) {
#     for (t in unique(dfZonePercentiles$harvestTreatment)) {
...
df <- dfZone %>%
    ...
    group_by(scenario, harvestTreatment, timestep) %>%
