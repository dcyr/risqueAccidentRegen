...
Mis à jour le `r format(Sys.time(), "%d %b %Y")`
...
cycleGlobal <- fireRegimeAttrib[which(fireRegimeAttrib$Zone_LN == "total"), 2]
##
...
Le territoire à l'étude est situé à l'est du lac Mistassini, tout juste au sud de la limite nordique des forêts attribuables. Il est d'une superficie totale de `r round(studyAreaTotal/1000000, 1)` Mha, dont `r 100 -round(flammableProp*100, 1)`% est constitué de grands cours d'eau. 


```{r fig.height = 6, fig.align = "center", echo=FALSE}
library(png)
library(grid)
img <- readPNG("figures/studyAreaLargeScale.png")
 grid.raster(img)
```
...
Ces simulations confirment que les régimes de feux sont presque parfaitement simulés pour le territoire à l'étude dans son ensemble (cycle global de `r cycleGlobal` ans).
