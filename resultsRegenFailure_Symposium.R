    ...
    for (i in c("all", "Global")) {
        if (i == "all") {
            ...
            labels <- labelDF %>%
                filter(#scenario == s,
                       productivity == prodNames) 
            fName <- paste0("immatureBurnsCover_", prodLabel, ".png")
            figHeight <- 6
        }
        if (i == "Global") {
            df <- dfCover %>%
        ...
        m <- ggplot(df, aes(x=timestep + 2015, y = 100*cumulpropBurned, group = ID)) +
            geom_line(colour = "black", alpha = 0.2) +#fill = "grey25"
            geom_line(aes(y = 100*p.500, group = 1),
                      colour = "lightblue",
                      linetype = 1, size = 1, alpha = 1) + #fill = "grey25"
        ...
        if (i == "Global") {
            m <- m +# facet_grid(productivity~harvestTreatment) +
                labs(caption = paste0("Sexual maturity thresholds (years) - Black Spruce: 50\n",
                                      "Jack Pine: 30"))
        }
        if (i == "all") {
        ...
        print(m + theme_dark() +
                  ...
                  geom_text(aes(x = xMin, y = yMax, group = NULL,
                            ...
                            data = labels,
                            hjust = 0, size = 3, fontface = 1, colour = "grey85"))
...
png(filename= paste0("immatureBurnsSummary.png"),
    width = 6, height = 4.5, units = "in", res = 600, pointsize=10)
options(scipen=999)
    ...
    print(m + theme_dark() +
              ...
              labs(title = "Cumulative proportion of productive area where immature stands were burned",
                   ...
                   caption = paste0("Sexual maturity thresholds (years) - Black Spruce: 50\n",
                                    "Jack Pine: 30"),
                   x = "",
                   y = "Cumulative area (%)"))
